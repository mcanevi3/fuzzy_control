...
vector::vector(std::initializer_list<real> list)
...
vector::vector(std::vector<real>::iterator begin, std::vector<real>::iterator end) {
    _size = static_cast<int>(std::distance(begin, end));
    _arr = new real[_size];
    std::copy(begin, end, _arr);
}
...
vector vector::operator-(const vector &rhs) const {
  if (_size != rhs._size)
    throw std::invalid_argument(
        "Vectors must be the same size for subtraction.");
  vector result(_size);
  for (int i = 0; i < _size; ++i)
    result._arr[i] = _arr[i] - rhs._arr[i];
  return result;
...
real vector::dot(const vector &rhs) const {
  if (_size != rhs._size)
    throw std::invalid_argument(
        "Vectors must be the same size for dot product.");
  real result = 0;
  for (int i = 0; i < _size; ++i)

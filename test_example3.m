...
% Fuzzy definitions
errorSet=-100:0.1:100;
negative_error=@(error)membership(error,"type","zshaped","start",-10,"stop",10);
zero_error=@(error)membership(error,"type","bell","center",0,"width",2,"slope",5);
positive_error =@(error)membership(error,"type","sshaped","start",-10,"stop",10);

deltaErrorSet=-1000:0.1:1000;
negative_delta_error=@(deltaError)membership(deltaError,"type","zshaped","start",-400,"stop",10);
zero_delta_error=@(deltaError)membership(deltaError,"type","bell","center",0,"width",20,"slope",3);
positive_delta_error =@(deltaError)membership(deltaError,"type","sshaped","start",-10,"stop",400);
